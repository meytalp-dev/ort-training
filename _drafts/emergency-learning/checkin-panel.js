/* ============================================================
   ...
   מה הרכיב עושה — לפי המצב הפעיל של המערכת (Resef.getMode):
     ...
     • חירום         →  4 אפשרויות בלחיצה אחת (מוכן / להתחיל לאט / אשמח לעזרה / לא יכול עכשיו),
                        עם אייקוני-קו (לא אימוג'י). "אשמח לעזרה"/"לא יכול עכשיו" מרימים
...
(function (global) {
  ...
  CheckInPanel.prototype._renderFour = function (ci) {
    var opts = (ci.options && ci.options.length) ? ci.options : [
      ...
      { id: 'help', label: 'אשמח לעזרה' }, { id: 'notnow', label: 'לא יכול/ה עכשיו' }
    ];
