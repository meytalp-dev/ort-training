...
window.RETZEF=(function(){
  ...
  /* כמה יחידות כבר הופקו במודולה (מ-produced.js אם נטען) — כדי לא לייצר מחדש */
  function producedN(subjId,pmN){
    var P=window.PRODUCED||{}, r=P[subjId+'|'+modNum(pmN)];
    return r&&r.n?r.n:0;
  }
  ...
  function buildPrompt(subj,mi){
    ...
    var done=producedN(subj.id,pm.n);
    ...
    return 'צור יחידות תוכן ל"מדף הרציפות" (רֶצֶף · _drafts/emergency-learning).\n\n'+
      ...
      (done>0 ? '   · ⚠ **'+done+' יחידות כבר קיימות בריפו** (`module-'+String(mi+1).padStart(2,'0')+'/`). **אל תייצר/י אותן מחדש ואל תדרוס/י אותן.** בדוק/בדקי מה כבר שם, והתחל/י מהתת-נושא הבא שטרם כוסה. נותרו כ-'+Math.max(0,subs.length-done)+' יחידות.\n' : '')+
      ...
      /* 17.7: כאן היה "מדפיס בצ'אט? רק אז מותר לעצור בין יחידות — כתוב 'כתבי המשך'".
       * זה סתר את הסעיף שמעל ואישר בדיוק את ההתנהגות שהוא אוסר: בעברית מ2 המחבר
       * חסם כתיבה, GPT נפל למסלול הצ'אט ועצר אחרי יחידה 1 — לפי ההוראה שלנו.
       * פתח-המילוט נשאר (עדיף שידפיס מאשר שיתחזה לדוחף), אבל בלי עצירה פר-יחידה. */
      '   · **מדפיס/ה בצ\'אט (אין דחיפה)?** גם אז — הפק/י את **כל** '+subs.length+' היחידות ברצף, באותה תשובה. **אל תעצור/י אחרי כל יחידה.** רק אם נגמר לך המקום ממש באמצע: סיים/י בשורה `— סיום יחידה N · כתבי המשך`, והמשך/י משם בהודעה הבאה.\n'+
      (done>0 ? '• התחל/י מיד מהיחידה הבאה שטרם הופקה. בלי הקדמות.\n' : '• התחל/י מיד ביחידה 1. בלי הקדמות.\n')+
