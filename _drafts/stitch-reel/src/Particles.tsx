...
export const Particles: React.FC = () => {
  ...
  // === BIG FLOATING ORBS ===
  const orbs = [
    { x: 120, y: 300, size: 60, color: STITCH.accent, speed: 14, drift: 50 },
    { x: 900, y: 600, size: 45, color: STITCH.accentLight, speed: 18, drift: 60 },
    { x: 500, y: 1200, size: 55, color: STITCH.teal, speed: 16, drift: 45 },
    { x: 200, y: 1600, size: 40, color: STITCH.accent, speed: 20, drift: 55 },
    { x: 800, y: 1000, size: 35, color: STITCH.accentLight, speed: 12, drift: 40 },
    { x: 950, y: 200, size: 30, color: STITCH.teal, speed: 22, drift: 50 },
  ];
  ...
  return (
    <>
      ...
      <svg style={{
        ...
        <defs>
          <radialGradient id="sBlob1grad">
            <stop offset="0%" stopColor={STITCH.accent} stopOpacity="0.7" />
            <stop offset="100%" stopColor={STITCH.accent} stopOpacity="0" />
          </radialGradient>
          <radialGradient id="sBlob2grad">
            <stop offset="0%" stopColor={STITCH.accentLight} stopOpacity="0.6" />
            <stop offset="100%" stopColor={STITCH.accentLight} stopOpacity="0" />
          </radialGradient>
          <filter id="sBlobBlur">
            <feGaussianBlur stdDeviation="50" />
          </filter>
        </defs>
        <path d={toSmoothPath(blob1Points)} fill="url(#sBlob1grad)" filter="url(#sBlobBlur)" />
        <path d={toSmoothPath(blob2Points)} fill="url(#sBlob2grad)" filter="url(#sBlobBlur)" />
      </svg>
      ...
      {rings.map((r, i) => {
        ...
        return (
          <div key={`ring-${i}`} style={{
            ...
            boxShadow: `0 0 ${r.size / 2}px ${STITCH.accent}25`,
      ...
      {diamonds.map((d, i) => {
        ...
        return (
          <div key={`d-${i}`} style={{
            ...
            width: d.size, height: d.size, background: STITCH.accentLight,
            transform: `rotate(${rotate}deg)`,
            opacity: 0.14,
            boxShadow: `0 0 ${d.size * 1.5}px ${STITCH.accentLight}30`,
            pointerEvents: "none",
          }} />
        );
      })}
