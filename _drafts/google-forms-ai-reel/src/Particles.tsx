...
export const Particles: React.FC = () => {
  ...
  // === FLOATING ORBS ===
  const orbs = [
    { x: 120, y: 300, size: 55, color: STYLE.accent, speed: 14, drift: 50 },
    { x: 900, y: 600, size: 45, color: STYLE.accentLight, speed: 18, drift: 60 },
    { x: 500, y: 1200, size: 50, color: STYLE.green, speed: 16, drift: 45 },
    { x: 200, y: 1600, size: 40, color: STYLE.greenLight, speed: 20, drift: 55 },
    { x: 800, y: 1000, size: 35, color: STYLE.skyBlue, speed: 12, drift: 40 },
    { x: 950, y: 200, size: 30, color: STYLE.accent, speed: 22, drift: 50 },
  ];
      ...
      <svg style={{
        ...
        <defs>
          <radialGradient id="blob1grad">
            <stop offset="0%" stopColor={STYLE.accent} stopOpacity="0.7" />
            <stop offset="100%" stopColor={STYLE.accent} stopOpacity="0" />
          </radialGradient>
          <radialGradient id="blob2grad">
            <stop offset="0%" stopColor={STYLE.green} stopOpacity="0.6" />
            <stop offset="100%" stopColor={STYLE.green} stopOpacity="0" />
          </radialGradient>
          <filter id="blobBlur">
      ...
      {rings.map((r, i) => {
        ...
        return (
          <div key={`ring-${i}`} style={{
            ...
            boxShadow: `0 0 ${r.size / 2}px ${STYLE.accent}25`,
      ...
      {diamonds.map((d, i) => {
        ...
        return (
          <div key={`d-${i}`} style={{
            ...
            width: d.size, height: d.size, background: STYLE.accentLight,
            transform: `rotate(${rotate}deg)`,
            opacity: 0.14,
            boxShadow: `0 0 ${d.size * 1.5}px ${STYLE.accentLight}30`,
            pointerEvents: "none",
          }} />
        );
      })}
