...
export const Particles: React.FC = () => {
  ...
  // === FLOATING ORBS — pink shades ===
  const orbs = [
    { x: 120, y: 300, size: 60, color: POMELLI.accent, speed: 14, drift: 50 },
    { x: 900, y: 600, size: 45, color: POMELLI.accentLight, speed: 18, drift: 60 },
    { x: 500, y: 1200, size: 55, color: POMELLI.accent, speed: 16, drift: 45 },
    { x: 200, y: 1600, size: 40, color: POMELLI.accentLight, speed: 20, drift: 55 },
    { x: 800, y: 1000, size: 35, color: POMELLI.accent, speed: 12, drift: 40 },
    { x: 950, y: 200, size: 30, color: POMELLI.accentLight, speed: 22, drift: 50 },
  ];

  // === GLOWING RINGS — pink borders ===
  const rings = [
  ...
  // === ROTATING DIAMONDS — accentLight pink ===
  const diamonds = [
    { x: 80, y: 400, size: 22 },
    { x: 950, y: 500, size: 18 },
  ...
  return (
    <>
      ...
      <svg style={{
        ...
        <defs>
          <radialGradient id="pblob1grad">
            <stop offset="0%" stopColor={POMELLI.accent} stopOpacity="0.7" />
            <stop offset="100%" stopColor={POMELLI.accent} stopOpacity="0" />
          </radialGradient>
          <radialGradient id="pblob2grad">
            <stop offset="0%" stopColor={POMELLI.accentLight} stopOpacity="0.6" />
            <stop offset="100%" stopColor={POMELLI.accentLight} stopOpacity="0" />
          </radialGradient>
          <filter id="pblobBlur">
            <feGaussianBlur stdDeviation="50" />
          </filter>
        </defs>
        <path d={toSmoothPath(blob1Points)} fill="url(#pblob1grad)" filter="url(#pblobBlur)" />
        <path d={toSmoothPath(blob2Points)} fill="url(#pblob2grad)" filter="url(#pblobBlur)" />
      </svg>
      ...
      {rings.map((r, i) => {
        ...
        return (
          <div key={`ring-${i}`} style={{
            ...
            border: `2px solid ${POMELLI.accent}`,
            boxShadow: `0 0 ${r.size / 2}px ${POMELLI.accent}25`,
      ...
      {diamonds.map((d, i) => {
        ...
        return (
          <div key={`d-${i}`} style={{
            ...
            width: d.size, height: d.size, background: POMELLI.accentLight,
            transform: `rotate(${rotate}deg)`,
            opacity: 0.14,
            boxShadow: `0 0 ${d.size * 1.5}px ${POMELLI.accentLight}30`,
            pointerEvents: "none",
          }} />
        );
      })}
